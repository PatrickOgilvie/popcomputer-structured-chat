...
test("transformed answer schemas can be reused before and after persistence", async () => {
  ...
  const chat = Chat.define({
    ...
    stages: [
      Brief,
      Stage.tools({ name: "results", instructions: ["Search"], tools: [Search] }),
    ],
    explorations: [Search],
  })
  ...
  await Effect.runPromise(
    Effect.gen(function* () {
      ...
      if (first.turn._tag === "Question") throw new Error("Expected results")
      ...
      if (next.turn._tag === "Question") throw new Error("Expected results")
...
test("repair guards and tools read the latest accepted answers on every turn", async () => {
  ...
  const guard = Model.guard({
    name: "budget_policy",
    check: () =>
      acceptedBudget.pipe(Effect.map((budget) => { beforeModel.push(budget) })),
    checkCall: () =>
      acceptedBudget.pipe(Effect.map((budget) => { beforeTool.push(budget) })),
  })
  const Search = Tool.define({
    name: "search",
    description: "Use the accepted budget",
  ...
  const model = Scenario.model(
    Scenario.answers(Budget, { budget: Scenario.quoted(500, { quote: "500" }) }),
    Scenario.call(Search, {}),
    Scenario.call(Search, {}),
    Scenario.repairs(Scenario.replace(Budget, "budget", 1000, { quote: "1000" })),
    Scenario.call(Search, {}),
  )

  await Effect.runPromise(
    Effect.gen(function* () {
      const first = yield* Chat.turn(chat, { sessionId: "budget", message: "Budget 500" })
      const next = yield* Chat.turn(chat, {
        sessionId: "budget",
        expectedRevision: first.revision,
        message: "Search again",
      ...
      for (const [reply, budget] of [[first, 500], [next, 500], [repaired, 1000]] as const) {
        if (reply.turn._tag === "Question") throw new Error("Expected results")
...
test("repair preserves transformed values through query execution and persistence", async () => {
  ...
  const model = Scenario.model(
    ...
    Scenario.repairs(Scenario.replace(Launch, "date", correctedDate, { quote: "October 1" })),
    Scenario.call(ReadLaunch, {}),
    Scenario.call(ReadLaunch, {}),
  )
  await Effect.runPromise(
    Effect.gen(function* () {
      const first = yield* Chat.turn(chat, { sessionId: "launch", message: "September 1" })
      const repaired = yield* Chat.turn(chat, {
        sessionId: "launch",
        expectedRevision: first.revision,
      ...
      for (const reply of [repaired, reloaded]) {
        if (reply.turn._tag === "Question") throw new Error("Expected launch result")
...
test("later collection validators can read previously accepted answers", async () => {
      ...
      title: Answer.semantic(Schema.String, {
        ...
        validate: () =>
          Tool.acceptedAnswer({
            ...
            field: "budget",
            schema: Budget.fields.budget.schema,
          }).pipe(Effect.map((budget) => { observed.push(budget) })),
  ...
  const model = Scenario.model(
    Scenario.answers(Budget, { budget: Scenario.quoted(500, { quote: "500" }) }),
    Scenario.answers(Project, { title: Scenario.quoted("website", { quote: "website" }) }),
    Scenario.call(Search, {}),
  )

  const reply = await Effect.runPromise(
    Chat.turn(chat, { sessionId: "project", message: "Build a website for 500" }).pipe(
...
test("missing and incompatible answer bindings fail before application work", async () => {
  ...
  expect(
    Result.isFailure(incompatible) && incompatible.failure.reason,
  ).toBe("incompatible")
  expect(executions).toBe(0)
})

test("inherited property names cannot masquerade as accepted answers", async () => {
  for (const reference of [{ stage: "constructor", field: "budget" }, { stage: "requirements", field: "constructor" }]) {
    const result = await Effect.runPromise(Tool.acceptedAnswer({ ...reference, schema: Schema.Number }).pipe(
      Effect.provideService(Tool.Context, { stages: { requirements: { accepted: {} } } }),
      Effect.result,
    ))

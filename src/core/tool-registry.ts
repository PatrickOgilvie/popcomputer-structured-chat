...
export type CommandContextSource = () => Effect.Effect<CommandExecutionContext>

/** @internal Closed dispatch for a tuple that may contain commands. */
export interface CompiledToolRegistry<Tools extends ModelToolTuple>
  extends ToolCallPlanner<Tools> {
  readonly execute: (
    call: ToolSetCall<Tools>,
    commandContext: CommandContextSource,
  ) => Effect.Effect<
...
export interface CompiledQueryToolRegistry<Tools extends ToolTuple>
  extends ToolCallPlanner<Tools> {
  readonly execute: (
    call: ToolSetCall<Tools>,
  ) => Effect.Effect<
...
export function compileToolRegistry<const Tools extends ModelToolTuple>(
  ...
  const select = (
    ...
    return tool === undefined
      ? Effect.fail(new InvalidToolCall({
          tool: name,
          reason: "unknown_tool",
          path: null,
        }))
  ...
  const parseRuntime = (input: JsonValue) =>
    Schema.decodeUnknownEffect(CallEnvelopeSchema)(input, {
      ...
      Effect.mapError(() => new InvalidToolCall({
        tool: null,
  ...
  const runRuntime = (
    ...
    Effect.flatMap((tool) => Effect.suspend(() => {
      if (tool.operation === "query") {
        return tool.execute(call.arguments)
      }
      if (commandContext === undefined) {
        return Effect.die(new Error("Command execution requires turn context"))
      }
  ...
  const decide = (
    ...
    if (call.name === repair.name) {
      ...
      return Decision.Repair({
        proposal: Fn.cast<typeof call.arguments, RepairProposal>(call.arguments),
      })
    }

...
The optional `@popcomputer/structured-chat/typesafe` entry point adds typed Noul, Choice, Score, and batched evaluation through an Effect service. Attach `TypeSafe.collection(fields, options)` to a collection stage to classify registered enum or boolean values, with explicit abstention and acceptance thresholds. Existing guards, evidence checks, validators, and session commits still determine acceptance.
